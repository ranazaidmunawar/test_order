...
class ProductController extends Controller
{
    ...
    public function product(Request $request, $domain)
    {
        ...
        $data['userBs'] = $bs;
        $data['userBe'] = $be;
        $data['rtl'] = $userCurrentLang->rtl;
        $data['currentLang'] = $userCurrentLang;
        ...
        $data['allLanguageInfos'] = Language::query()
            ->where('user_id', $user->id)
            ->get();
        $data['langs'] = $data['allLanguageInfos'];
        $userMenu = Menu::query()
            ->where('user_id', $user->id)
            ->where('language_id', $userCurrentLang->id)
            ->first();
    ...
    public function productDetails($domain, $slug, $id)
    {
        ...
        $userMenu = Menu::query()
            ...
            ->where('language_id', $userCurrentLang->id)
            ->first();
        $data['userMenus'] = $userMenu ? json_decode($userMenu->menus, true) : [];
    ...
    public function cart($domain)
    {
        ...
        $data = [
            ...
            'allLanguageInfos' => \App\Models\User\Language::query()
                ->where('user_id', $user->id)
                ->get(),
            'langs' => \App\Models\User\Language::query()
                ->where('user_id', $user->id)
                ->get(),
            'apopups' => Popup::query()
                ->where('user_id', $user->id)
        ...
        return view('user-front.product.cart', $data);
    }
